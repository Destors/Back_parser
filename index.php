...
$capsule->addConnection(['driver' => 'mysql', 'host' => 'localhost', 'database' => 'parserDB', 'username' => 'root', 'password' => '', 'charset' => 'utf8', 'collation' => 'utf8_general_ci', 'prefix' => '', ]);
$capsule->setAsGlobal();
...
do {
    $isContent = true;
    $params = ['query' => 'веб-студия', 'page' => $pageIndex];
    ...
    $crawler->filter('tr.projects-table__row')->each(function (Crawler $node, $i) use (&$list, $baseUrl) {
        ...
        $list[$i]['popularity'] = 0;
        $list[$i]['views'] = 0;
        $node->filter('.link_catalogue-site-link')->each(function (Crawler $node) use ($i, &$list, $baseUrl) {
            $_node = $node->getNode(0);
        ...
        $node->filter('.projects-table__cell[data-content="visitors"] .projects-table__textline')->each(function (Crawler $node) use ($i, &$list, $baseUrl) {
            $_node = $node->getNode(0);
            $vstrs = $_node->nodeValue;
            $vstrs = preg_replace("/[^x\d|*\.]/", "", $vstrs);
            $list[$i]['visitors'] = $vstrs;
        });
        $node->filter('.projects-table__cell[data-content="views"] .projects-table__textline')->each(function (Crawler $node) use ($i, &$list, $baseUrl) {
            $_node = $node->getNode(0);
            $vws = $_node->nodeValue;
            $vws = preg_replace("/[^x\d|*\.]/", "", $vws);
            $list[$i]['views'] = $vws;
        });
        $node->filter('.projects-table__cell[data-content="popularity"] .projects-table__textline')->each(function (Crawler $node) use ($i, &$list, $baseUrl) {
            $_node = $node->getNode(0);
            $pop = $_node->nodeValue;
    ...
    foreach ($list as $i => $item) {
        ...
        $model = \parser\models\Parse::query()->where('url', '=', $item['url'])->where('uid', '=', $item['uid'])->first();
        if ($model === null) {
